...
import WordPracticeSelector from './WordPracticeSelector';
...
const WordPractice: React.FC<WordPracticeProps> = ({
  ...
  const handleConfigChange = useCallback((newConfig: DictationConfig) => {
    // 确保始终为单词练习
    const wordConfig = { ...newConfig, practice_type: 'word' as const };
    setConfig(wordConfig);
  }, []);
  ...
  const handleStartPractice = useCallback(async (practiceConfig: DictationConfig) => {
    try {
      ...
      // 确保是单词练习类型
      const wordConfig = { ...practiceConfig, practice_type: 'word' as const };
      
      // 获取练习句子
      const practiceMessages = await enhancedDictationService.getPracticeSentences(wordConfig);
      setSentences(practiceMessages);
      
      if (practiceMessages.length === 0) {
        message.error('没有找到合适的练习单词，请调整设置');
        return;
      }
      ...
      message.success('单词练习开始！请听音频并填写空白处');
      ...
      Modal.confirm({
        ...
        onOk: async () => {
          try {
            ...
            message.success(`开始第 ${nextIndex + 1} 个单词`);
          } catch (error) {
  ...
  const handlePreviousSentence = useCallback(async () => {
    ...
    if (prevIndex >= 0) {
      try {
        ...
        message.success(`切换到第 ${prevIndex + 1} 个单词`);
      } catch (error) {
  ...
  return (
    <div className={`dictation-practice word-practice ${className}`}>
